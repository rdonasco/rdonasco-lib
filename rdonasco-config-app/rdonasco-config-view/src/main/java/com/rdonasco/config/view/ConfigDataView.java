...
public class ConfigDataView extends VerticalLayout implements ControlledView
{
	...
	private void configureTreeTableActionHandlers()
	{
		getConfigTreeTable().addActionHandler(new Action.Handler()
		{
			...
			{
				Action[] actions;
				if (target instanceof ConfigElementVO)
				{
					ConfigElementVO configElement = (ConfigElementVO) target;
				...
				else if (target instanceof ConfigAttributeVO)
				{
			...
			public void handleAction(Action action, Object sender, Object target)
			{
				try
				{
					...
					{
						ConfigElementVO newElement = getConfigDataContainer().createNewConfigElement();
						getConfigTreeTable().select(newElement);
					}
					else if (action == ACTION_ADD_SUB_ELEMENT && target instanceof ConfigElementVO)
					{
						ConfigElementVO parentElement = (ConfigElementVO) target;
						ConfigElementVO newSubElement = getConfigDataContainer().createNewSubConfigElement(parentElement);
						getConfigTreeTable().setCollapsed(parentElement, false);
						getConfigTreeTable().select(newSubElement);
					}
					else if (action == ACTION_ADD_ATTRIBUTE && target instanceof ConfigElementVO)
					{
						ConfigElementVO parentElement = (ConfigElementVO) target;
						ConfigAttributeVO newAttribute = getConfigDataContainer().createNewAttribute(parentElement);
						getConfigTreeTable().setCollapsed(parentElement, false);
						getConfigTreeTable().select(newAttribute);
					}
					else if (action == ACTION_DELETE_ITEM)
					{
						getConfigDataContainer().removeConfigData((ConfigDataVO) target);
					}
					else if (action == ACTION_EXPAND_ALL && target instanceof ConfigElementVO)
					{
	...
	private void setClickedItemForEditMode(ItemClickEvent event)
	{
		...
		if (ConfigDataContainer.PROPERTY_CONFIG_VALUE.equals(
				event.getPropertyId()) && event.getItemId() instanceof ConfigAttributeVO)
		{
	...
	private DefaultFieldFactory prepareFieldFactory()
	{
		DefaultFieldFactory fieldFactory = new DefaultFieldFactory()
		{
					...
					Component uiContext)
			{
				...
				if (beanItem.getBean() instanceof ConfigElementVO)
				{
